<div>
    ...
    <div class="flex justify-left mb-4">
        ...
        <flux:input placeholder="{{ __('messages.Enter user name or email') }}" wire:model.live="search" class="w-2xl! ml-2!" />
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-6 py-3" wire:click="order('type')">
                    {{ __('messages.Type') }} <i class="fa-solid fa-magnifying-glass"></i>
                ...
                <th scope="col" class="px-6 py-3">
                    {{ __('messages.Role') }} <i class="fa-solid fa-magnifying-glass"></i>
                </th>
        ...
        <tbody>
            @foreach ($users as $user)
                <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200">
                    ...
                    <td class="px-6 py-4 text-center">
                        {{ $user->getRoleNames()->first() ?? __('messages.No role assigned') }}
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex space-x-2 justify-center">
                            @can('admin.users.show')
                                <a href="{{ route('admin.users.show', $user) }}" class="btn btn-warning text-xs">{{ __('messages.Show') }}</a>
                            @endcan
                            @can('admin.users.edit')
                                <a href="{{ route('admin.users.edit', $user) }}" class="btn btn-secondary text-xs">{{ __('messages.Edit') }}</a>
                            @endcan
