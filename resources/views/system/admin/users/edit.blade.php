<x-layouts.admin>
    ...
    <div class="card">
        <p class="text-2xl mb-4">{{  __('messages.Edit User') }}</p>
        <form action="{{ route('admin.users.update', $user) }}" method="POST">
            ...
            <flux:field>
                ...
                <flux:input name='name' placeholder="{{ __('messages.Enter the name') }}" value="{{ old('name', $user->name) }}"/>
                <flux:error name="name" />
            </flux:field>

            {{-- <flux:field>
                <flux:label class="mt-2!">{{ __('messages.Email') }}</flux:label>
                <flux:input name='email' placeholder="{{ __('messages.Enter the email') }}" value="{{ old('email', $user->email) }}"/>
           ...
           <flux:label class="mt-2!">{{ __('messages.Email') }}</flux:label>
            ...
            <flux:field>
                ...
                <select name="state_id" class="select">
                    ...
                    @foreach($states as $state)
                        <option value="{{ old('state_id', $user->state_id) }}" @selected(old('state_id', $user->state_id) == $state->id)>
            ...
            <flux:field>
                ...
                <flux:input name='address' placeholder="{{ __('messages.Enter the address') }}" value="{{ old('address', $user->address) }}"/>
                <flux:error name="address" />
            </flux:field>

            <flux:field>
                <flux:label class="mt-2!">{{ __('messages.Phone Number') }}</flux:label>
                <flux:input name='phone' placeholder="{{ __('messages.Enter the phone number') }}" value="{{ old('phone', $user->phone) }}"/>
                <flux:error name="phone" />
            </flux:field>

            <flux:field>
                <flux:label class="mt-2!">{{ __('messages.ZIP Code') }}</flux:label>
                <flux:input name='zip_code' placeholder="{{ __('messages.Enter the zip code') }}" value="{{ old('zip_code', $user->phone) }}"/>
            ...
            <div x-data="{ userType: '{{ old('type', $user->type ?? 'Admin') }}' }">
                ...
                <flux:field>
                    ...
                    <input 
                        ...
                        :class="{ 'opacity-50 cursor-not-allowed': userType !== 'Person' }"
                        class="input"
                ...
                <flux:field>
                    ...
                    <input 
                        ...
                        :class="{ 'opacity-50 cursor-not-allowed': userType !== 'Company' }"
                        class="input"
            ...
            {{-- <flux:field>
                ...
                <flux:input name='social_security' placeholder="{{ __('messages.Enter the social security number') }}" value="{{ old('social_security', $user->social_security) }}"/>
                <flux:error name="social_security" />
            </flux:field>

            <flux:field>
                <flux:label class="mt-2!">{{ __('messages.Federal Identification Number') }}</flux:label>
                <flux:input name='fin' placeholder="{{ __('messages.Enter the federal identification number') }}" value="{{ old('fin', $user->fin) }}"/>
                <flux:error name="fin" />
            </flux:field>
