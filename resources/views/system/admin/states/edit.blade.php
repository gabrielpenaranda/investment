<x-layouts.admin>
    ...
    <div class="card">
        <p class="text-2xl mb-4">{{  __('messages.Edit State') }}</p>
        <form action="{{ route('admin.states.update', $state) }}" method="POST">
            ...
            <flux:field>
                ...
                <flux:input name='code' placeholder="{{ __('messages.Enter the code') }}" value="{{ old('code', $state->code) }}"/>
             ...
             <flux:field>
                ...
                <select name="country_id" class="select" placeholder="{{ __('messages.Select country') }}">
                    ...
                    @foreach($countries as $country)
                        <option value="{{ old('country_id', $country->id) }}" @selected($country->id === $state->country_id)>
                                {{ $country->name }}
                        </option>
                    @endforeach
                </select>
