    ...
    <div class="card">
        ...
        <form action="{{ route('admin.investments.update', $investment) }}" method="POST">
            ...
            <flux:field>
                ...
                @endif
                <flux:input name='investment_amount'  placeholder="{{ __('messages.Enter the investment amount') }}" value="{{ old('investment_amount', $formatter->format($investment->investment_amount)) }}"/>
            ...
            <flux:label for="opening_date_display" class="mt-2!">{{ __('messages.Opening Date') }}</flux:label>
             <p class="w-full border border-zinc-200 rounded-md p-2 focus:ring-2 focus:ring-zinc-500 focus:border-ring-500 text-red-500">
                @php
                    $opening_date = app()->getLocale() === 'es' ? date("d/m/Y", strtotime($investment->opening_date)) : date("m/d/Y", strtotime($investment->opening_date));
                @endphp
                {{ $opening_date }}
            </p>
